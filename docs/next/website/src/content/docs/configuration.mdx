...
While an agent is working, its state icon animates as a spinner: braille frames with `dots`, rotating quarter circles with `symbols`. Blocked, done, idle, and unknown icons are always static. To keep the working icon static too, configure:
